...
def process_file(files):
    ...
    for file in files:
        ...
        if file_extension == '.pdf':
            ...
            st.warning(f'Skipping file "{file.name}": Unsupported file format. Please upload a PDF or DOC/DOCX file.')
            continue

        preprocessed_text = preprocess_resume(text)
        vectorized_text = tfidf_vectorizer.transform([preprocessed_text])
        prediction = model.predict(vectorized_text)
        predictions.append((file.name, prediction[0]))
...
def main():
    ...
    files = st.file_uploader('Upload File', type=['pdf', 'docx', 'doc'], accept_multiple_files=True)

    if files is not None:
        predictions = process_file(files)
        for file_name, prediction in predictions:
            st.write(f'File: {file_name}, Prediction: {prediction}')
